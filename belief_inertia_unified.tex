...
This four-part structure has transparent physical meaning. The term $\bar{\Lambda}_{pi}$ represents \textbf{prior inertia}, the resistance arising from the cost of deviating from deep expectations. The term $\Lambda_{oi}$ represents \textbf{sensory inertia}, grounding through observation whereby precise senses anchor beliefs. The term $\sum_k \beta_{ik}\tilde{\Lambda}_{qk}$ represents \textbf{incoming social inertia}, the effect of being pulled toward confident neighbors. Finally, the term $\sum_j \beta_{ji}\Lambda_{qi}$ represents \textbf{outgoing social inertia}, the recoil from exerting influence on others.
...
The term $\bar{\Lambda}_{pi}$ represents \textbf{bare mass}, providing inertia against deviation from the prior. The term $\sum_k \beta_{ik}\tilde{\Lambda}_{qk}$ represents \textbf{incoming relational mass}, the inertia from being ``pulled'' by neighbors. The term $\sum_j \beta_{ji}\Lambda_{qi}$ represents \textbf{outgoing relational mass}, the inertia from ``pulling'' neighbors (recoil). Finally, $\Lambda_{o_i}$ represents \textbf{sensory mass}, the inertia from grounding in observations.
...
This construction guarantees a normalized joint distribution by design, in contrast to unnormalized Markov random fields of the form $p(\{q_i\}) \propto \exp[-\sum_{ik} \psi_{ik}(q_i, q_k)]$ where the partition function is generally intractable.
...
The full joint generative model over all belief means and agreement variables is:
\begin{equation}
p(\{\mu_i\}, \{z_{ik}\}) = \left[\prod_i p_i(\mu_i)\right] \times \left[\prod_{i,k} \mathcal{N}(z_{ik}; \mu_i, \Lambda_{ik}^{-1}) \cdot \mathcal{N}(z_{ik}; \Omega_{ik}\mu_k, \Lambda_{ik}^{-1})\right]
\end{equation}
where $p_i(\mu_i) = \mathcal{N}(\mu_i; \bar{\mu}_i, \bar{\Sigma}_i)$ is agent $i$'s local prior. This joint distribution is properly normalized since each factor is a normalized Gaussian and the agreement variables mediate between disjoint variable sets.

\subsection{Integration Yields KL Coupling}

Under a mean-field posterior approximation $q(\{\mu_i\}) = \prod_i q_i(\mu_i)$ with Gaussian factors $q_i = \mathcal{N}(\mu_i; \mu_{q,i}, \Sigma_{q,i})$, the variational free energy takes the form:
\begin{equation}
\mathcal{F} = \mathbb{E}_q[\log q] - \mathbb{E}_q[\log p(\{\mu_i\})] - \mathbb{E}_q[\log p(o \mid \{\mu_i\})]
\end{equation}

Expanding using the joint model and integrating out the agreement variables yields:
\begin{equation}

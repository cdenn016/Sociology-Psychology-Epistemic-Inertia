...
Under the Hamiltonian ansatz, mixture beliefs exhibit considerably richer dynamics than their unimodal counterparts. The extended state space---comprising weights, means, and covariances for multiple components---creates a manifold with complex topology, and the belief trajectories that unfold across this manifold display phenomena that have no analogue in single-Gaussian dynamics.

Perhaps the most striking of these phenomena is mode competition, which emerges when two or more mixture components carry comparable weights. In this regime, the belief system becomes exquisitely sensitive to perturbation. Small fluctuations in evidence or minor shifts in contextual information can trigger oscillatory transfers of probability mass between competing hypotheses. The agent, in effect, hesitates---the belief momentum carries weight toward one interpretation, overshoots equilibrium, and swings back toward the alternative. This oscillation may damp toward a stable allocation or, under sustained ambiguous evidence, persist indefinitely. Such dynamics offer a natural account of the vacillation and uncertainty that characterizes belief in genuinely ambiguous situations, where the evidence admits multiple coherent interpretations and the mind cycles between them without settling.

The opposite extreme is mode collapse, which occurs under conditions of high epistemic friction. When damping dominates the dynamics---whether through cognitive fatigue, time pressure, or strong prior commitment to parsimony---mixture beliefs tend to simplify. Probability mass drains from minority components and consolidates into a single dominant mode. In the limit of complete collapse, the mixture effectively becomes a unimodal Gaussian, and the dynamics reduce to those analyzed in the main text. Mode collapse thus represents a kind of epistemic simplification, potentially adaptive when cognitive resources are scarce but potentially pathological when it prematurely forecloses genuine alternatives. The phenomenon maps onto psychological observations about premature closure and the tendency, under stress or cognitive load, to retreat from nuanced probabilistic assessments to simpler categorical judgments.

In counterpoint to collapse stands mode creation, the process by which a unimodal belief fragments into multiple distinct hypotheses. This occurs when evidence strongly favors a bimodal or multimodal interpretation---for instance, when observations cluster around two distinct values rather than scattering around a single mean. Geometrically, mode creation corresponds to the belief trajectory crossing a bifurcation point in the free energy landscape. What was a single basin of attraction splits into two, and the belief distribution must follow, developing distinct components to track the newly separated hypotheses. Mode creation captures moments of conceptual differentiation: the realization that what seemed a single phenomenon actually comprises distinct subtypes, or the dawning recognition that a trusted source may be unreliable, spawning competing hypotheses about their testimony. The dynamics of bifurcation determine how smoothly or abruptly this differentiation occurs, with low-friction systems permitting gradual divergence and high-friction systems potentially undergoing sudden splitting when accumulated evidence finally overcomes inertial resistance.
...
The theoretical elegance of infinite-dimensional belief dynamics encounters immediate practical obstacles: no finite computer can represent an arbitrary probability density exactly, and the integrals defining the Fisher-Rao metric become analytically intractable for all but the simplest distributions. Bridging the gap between theory and implementation requires systematic approximation strategies, each with characteristic strengths and limitations.

One natural approach represents the belief distribution through a finite collection of weighted samples---a particle approximation. Rather than storing a functional form, the system maintains a population of points in belief space, each carrying equal probability mass. The empirical distribution defined by these particles approximates the true belief, with accuracy improving as the particle count grows. Within this representation, the Fisher-Rao metric induces a kernel structure on particle positions, measuring how changes in one particle's location affect the overall distribution's information geometry. Stein variational gradient descent exploits this structure to provide a practical algorithm: particles move according to a combination of the evidence gradient and a repulsive interaction that maintains distributional spread. The resulting dynamics approximate the true information-geometric flow while remaining computationally tractable. This approach has found particular success in Bayesian inference applications, where it offers a middle path between the flexibility of fully nonparametric methods and the efficiency of parametric approximations.

A second approach exploits the expressive power of deep neural networks through normalizing flows. Here the belief distribution is parameterized implicitly, as the image of a simple base distribution (typically a standard Gaussian) under a learned invertible transformation. The neural network defines this transformation, and its parameters become the coordinates of belief space. The Fisher information of these parameters defines the mass matrix for epistemic dynamics, capturing how changes in network weights affect the represented distribution. This approach offers remarkable flexibility---normalizing flows can approximate essentially arbitrary continuous distributions---at the cost of increased computational complexity and potential optimization difficulties. The resulting dynamics unfold not in the abstract space of probability densities but in the concrete space of neural network parameters, grounding the geometric theory in implementable algorithms.

A third strategy, perhaps the most directly connected to the finite-dimensional theory developed above, projects general distributions onto tractable parametric families. Given an arbitrary belief, one finds the Gaussian, mixture model, or exponential family member that best approximates it according to some divergence criterion (typically the Kullback-Leibler divergence). The dynamics then proceed within this tractable family, with periodic re-projection as the belief evolves. This variational truncation sacrifices some fidelity to the true infinite-dimensional dynamics in exchange for the full analytical machinery of finite-dimensional information geometry. The approach is most appropriate when the true belief remains close to the approximating family throughout its evolution, and least appropriate when the dynamics would naturally drive the belief toward shapes that the parametric family cannot capture.

\subsection{Gauge Structure for Non-Gaussian Beliefs}

The gauge-theoretic structure introduced for Gaussian beliefs in social networks admits natural generalization to richer belief families, though the geometric picture grows correspondingly more complex. Recall that the transport operators $\Omega_{ij}$ encode how one agent's belief coordinates relate to another's, compensating for arbitrary choices of reference frame and ensuring that the coupled dynamics remain consistent across the network. For Gaussian beliefs, these operators were rotations---elements of $\mathrm{SO}(d)$---reflecting the freedom to orient coordinate axes arbitrarily within each agent's belief space. The generalization to non-Gaussian beliefs expands this gauge freedom in ways that illuminate the deep connection between belief geometry and symmetry.

For beliefs within exponential families, the gauge transport acts on the natural parameters that characterize each distribution. A transport operator $\Omega$ maps parameters $\theta$ to transformed parameters $\Omega \cdot \theta$, with the constraint that this action must preserve the structure of the parameter space. For instance, if the natural parameters must satisfy positivity constraints (as for Poisson rate parameters) or sum-to-one constraints (as for categorical probabilities), the transport must respect these restrictions. The resulting gauge group is the group of transformations that preserve the parameter space geometry---typically a proper subgroup of the general linear group, determined by the specific exponential family under consideration. The connection forms and curvatures that govern parallel transport inherit corresponding structure, encoding how the notion of ``same belief'' varies across agents with different parameterization conventions.

Mixture models introduce additional gauge freedom beyond the within-component transformations. Because the labeling of mixture components is arbitrary---component one for agent $i$ need not correspond to component one for agent $j$---transport operators can permute components, swapping which hypothesis carries which label. Beyond permutation, transport can independently rotate or transform the mean and covariance of each component, and can adjust the weight simplex coordinates by any transformation that preserves the probability normalization. The full gauge group for $M$-component mixtures thus includes the symmetric group $S_M$ (permuting labels), combined with the within-component transformation groups for means and covariances. This rich structure reflects the genuine ambiguity in how different agents may carve up the same hypothesis space into labeled alternatives, and the gauge connection encodes how these different carving schemes relate across the social network.

In the most general case of arbitrary probability distributions, the gauge freedom expands dramatically. Transport operators become diffeomorphisms of the sample space itself---smooth invertible maps that transform one set of coordinates into another while preserving the underlying measurable structure. Under such a diffeomorphism $\Omega$, a distribution $q(x)$ transforms to $q(\Omega^{-1}(x)) |\det D\Omega^{-1}|$, the familiar change-of-variables formula from probability theory. The gauge group is thus the group of measure-preserving diffeomorphisms (or, more precisely, the group of diffeomorphisms modulo those that leave all distributions invariant), an infinite-dimensional Lie group whose structure is the subject of ongoing mathematical research. The corresponding gauge connection inherits this infinite-dimensional character, and its curvature encodes genuinely topological information about the space of probability distributions. While this full generality presents formidable technical challenges, it reveals that the gauge structure of belief dynamics is not an artifact of the Gaussian simplification but a fundamental feature of how epistemic states compose across social networks.

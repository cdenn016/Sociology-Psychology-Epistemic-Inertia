...
\subsection{Belief Overshoot and Perseverance}\label{sec:overshoot}Presently, research treats belief perseverance---the tendency to maintain beliefs despite discrediting evidence---as a flaw in information processing. Epistemic momentum yields an alternative perspective: perseverance is the natural dynamical consequence of beliefs possessing inertia.Consider an agent moving toward a conclusion with velocity $\dot{\mu}$ who encounters a sudden shift in evidence (a new equilibrium). A massless agent would instantly reverse course. An inertial agent, however, possesses kinetic energy that must be converted into potential energy before they can stop and reverse.In the low-damping limit, energy conservation on the quadratic free energy landscape dictates that the initial kinetic energy converts to potential energy at the point of maximum overshoot:
...
The damping coefficient $\gamma$ represents the rate of gradient-based updating—equivalently, the inverse learning rate in standard formulations. In the overdamped limit $\gamma \gg 2\sqrt{KM}$, dynamics reduce to the familiar $\dot{\mu} = -\gamma^{-1}\nabla F$, recovering standard variational inference.

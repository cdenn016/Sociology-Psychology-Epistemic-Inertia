...
\begin{abstract}
Phenomenological mass/spring models of belief dynamics have proven empirically successful across psychology, neuroscience, and sociology, yet lack theoretical justification. We show that variational free energy minimization on statistical manifolds naturally identifies the Fisher information metric as an inertial mass tensor---with confident beliefs resisting change and uncertain beliefs updating readily. We define \textbf{proper time} as information-theoretic arc length, providing a principled notion of ``how long'' belief updates take that depends on agent precision. Interactions between agents are encoded via gauge-invariant attention mechanisms enabling communication across reference frames.

The Hamiltonian formulation of belief dynamics (with momentum and oscillation) is presented as a \textbf{pregnant ansatz}: while the Fisher metric provides geometry, second-order dynamics are not derived but postulated. Crucially, the classical sociological models we derive---DeGroot social learning, Friedkin-Johnsen opinion dynamics, bounded confidence, echo chambers, confirmation bias, Social Impact Theory, and diffusion of innovations---depend \emph{only} on the overdamped (gradient flow) limit, not on the inertial ansatz. These derivations are thus geometrically necessary consequences of the framework.

In the overdamped limit, standard Bayesian updating emerges; in underdamped regimes, the ansatz predicts belief oscillation, overshooting, and epistemic resonance. The framework offers a geometric explanation for confirmation bias, belief perseverance, and opinion polarization as epistemic inertia rather than irrationality, unifying disparate observations under a single principled theory.
\end{abstract}

...
Crucially, \textbf{flat gauge} ($\Omega_{ij} = I$ for all agent pairs) recovers standard consensus models where all agents share a common reference frame. All derivations in this paper---the mass formula, classical model limits, and dynamical predictions---hold in the flat gauge limit. The general gauge structure is mathematical scaffolding that could accommodate reference frame heterogeneity (e.g., cultural or linguistic translation effects) but is not empirically motivated here. We retain it for generality while emphasizing that the core results do not depend on non-trivial gauge.

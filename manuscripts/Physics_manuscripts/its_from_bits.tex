...
In our associated bundle geometry we have shown that the variational free energy then becomes
...
The observation log-likelihood term, $\mathbb{E}_{q_i}[\log p_i(o \mid c)]$, represents agents reconciling their beliefs with sensory data according to their model $p_i$. We have shown previously that \cite{Dennis2025, Dennis2025a, Dennis2025b}
...
In this view, observables are agents with their own beliefs and priors.

\subsection{Hierarchical Emergence}

As we have previously shown, under gradient descent of \eqref{eq:free_energy_functional}, natural hierarchical evolution unfolds as agents interact and reach consensus via $\beta_{ij}$ alignment. When local clusters of agents align their beliefs/priors (low pairwise KL divergence), higher-order meta-agents emerge as coarse-grained collective variables as a form of dynamical renormalization ~\cite{Dennis2025a}

In our implementation, cross-scale couplings allow meta-agent beliefs to propagate downward as priors for its constituent agents, completing an Ouroboros tower of self-consistent, self-referential evolution, (i.e. a fully participatory universe of agents, beliefs, priors, and gauge frames) where the "top-observer" percolates beliefs down to lower levels and emergent levels bloom via dynamic renormalization.
...
The framework presented in Section~\ref{sec:intro} defines a variational free energy $\mathcal{F}$ that serves as the potential for agent dynamics. We now show that promoting this to a full Lagrangian theory reveals a kinetic energy term hidden in the second-order structure of KL divergence, and that this kinetic term identifies inertial mass with statistical precision. We work in the quasi-static approximation where prior parameters $(\bar{\mu}_i, \bar{\Sigma}_i)$ evolve slowly relative to beliefs $(\mu_i, \Sigma_i)$.
...
Given the kinetic energy $\mathcal{T}$ and the potential $\mathcal{F}$, we construct the complete Lagrangian and Hamiltonian formulations for epistemic dynamics.

\subsection{The Lagrangian and Hamiltonian}

The Lagrangian is $\mathcal{L} = \mathcal{T} - \mathcal{F}$, with action $S = \int_{\tau_0}^{\tau_1} \mathcal{L} \, d\tau$ where $\tau$ represents proper time on the statistical manifold. The conjugate momenta are:
\begin{align}
\pi_i^\mu &= \frac{\partial \mathcal{L}}{\partial \dot{\mu}_i} = \sum_k [M_{\mu\mu}]_{ik} \dot{\mu}_k + \sum_k [C^{\mu\Sigma}]_{ik} \dot{\Sigma}_k \\
\Pi_i^\Sigma &= \frac{\partial \mathcal{L}}{\partial \dot{\Sigma}_i} = \sum_k [C^{\mu\Sigma}]_{ki}^T \dot{\mu}_k + \sum_k [M_{\Sigma\Sigma}]_{ik} \dot{\Sigma}_k
\end{align}
The Hamiltonian is:
\begin{equation}
\mathcal{H} = \sum_i \left(\pi_i^\mu \cdot \dot{\mu}_i + \mathrm{tr}[\Pi_i^\Sigma \dot{\Sigma}_i]\right) - \mathcal{L} = \frac{1}{2}\langle\pi, M^{-1}\pi\rangle + \mathcal{F}[\xi]
\label{eq:hamiltonian}
\end{equation}
where $\pi = (\pi^\mu, \Pi^\Sigma)$ is the full momentum vector.

\subsection{Hamilton's Equations}

The equations of motion are:
\begin{align}
\dot{\mu}_i &= \sum_k [M^{-1}]_{ik}^{\mu\mu} \pi_k^\mu + \sum_k [M^{-1}]_{ik}^{\mu\Sigma} \Pi_k^\Sigma \label{eq:hamilton_mu}\\
\dot{\Sigma}_i &= \sum_k [M^{-1}]_{ik}^{\Sigma\mu} \pi_k^\mu + \sum_k [M^{-1}]_{ik}^{\Sigma\Sigma} \Pi_k^\Sigma \label{eq:hamilton_Sigma}\\
\dot{\pi}_i^\mu &= -\frac{\partial \mathcal{F}}{\partial \mu_i} - \frac{1}{2}\pi^T \frac{\partial M^{-1}}{\partial \mu_i}\pi \label{eq:hamilton_pi_mu}\\
\dot{\Pi}_i^\Sigma &= -\frac{\partial \mathcal{F}}{\partial \Sigma_i} - \frac{1}{2}\pi^T \frac{\partial M^{-1}}{\partial \Sigma_i}\pi \label{eq:hamilton_pi_Sigma}
\end{align}
The second terms in \eqref{eq:hamilton_pi_mu} and \eqref{eq:hamilton_pi_Sigma} are geodesic forces encoding manifold curvature.

\subsection{Force Decomposition}

The potential forces on mean parameters decompose into four physically distinct contributions:
\begin{equation}
-\frac{\partial \mathcal{F}}{\partial \mu_i} = -\bar{\Lambda}_{p_i}(\mu_i - \bar{\mu}_i) - \sum_k \beta_{ik}\tilde{\Lambda}_{q_k}(\mu_i - \tilde{\mu}_k) - \sum_j \beta_{ji}\Lambda_{q_i}\Omega_{ji}^T(\tilde{\mu}_i^{(j)} - \mu_j) - \Lambda_{o_i}(\mu_i - o_i)
\label{eq:force_decomposition}
\end{equation}
representing prior restoring, consensus, reciprocal, and sensory evidence forces respectively.

The potential forces on covariance parameters are:
\begin{equation}
-\frac{\partial \mathcal{F}}{\partial \Sigma_i} = -\frac{1}{2}(\bar{\Lambda}_{p_i} - \Lambda_{q_i}) - \sum_k \frac{\beta_{ik}}{2}(\tilde{\Lambda}_{q_k} - \Lambda_{q_i}) - \sum_j \frac{\beta_{ji}}{2}\Omega_{ji}^T(\tilde{\Lambda}_{q_i}^{(j)} - \tilde{\Lambda}_{q_i}^{(j)}\Sigma_j\tilde{\Lambda}_{q_i}^{(j)})\Omega_{ji} - \frac{1}{2}\Lambda_{o_i}
\end{equation}

Combining \eqref{eq:hamilton_mu}--\eqref{eq:hamilton_pi_mu} in the decoupled limit yields an informational Newton's second law:
\begin{equation}
\boxed{M_i \ddot{\mu}_i = F_i}
\label{eq:newton}
\end{equation}

\subsection{Momentum Current and Continuity}

Between agents, the momentum current is $J_{k \to i} = \beta_{ik}\tilde{\Lambda}_{q_k}(\tilde{\mu}_k - \mu_i)$. The continuity equation with dissipation $\gamma_i$ becomes:
\begin{equation}
\dot{\pi}_i^\mu + \gamma_i \dot{\mu}_i + \bar{\Lambda}_{p_i}(\mu_i - \bar{\mu}_i) + \Lambda_{o_i}(\mu_i - o_i) = \sum_k J_{k \to i}
\end{equation}
The sensory term acts as an additional anchoring force distinct from social momentum currents.

\subsection{Conservation Laws}

By Noether's theorem, time translation invariance implies energy conservation $d\mathcal{H}/dt = 0$, gauge invariance ($\phi_i \to \phi_i + \xi$) implies conservation of gauge charge, and translation invariance on $\mathcal{C}$ (if present) implies conservation of total informational momentum $P_{\mathrm{total}} = \sum_i \int_{\mathcal{C}} \pi_i(c)\, dc$.
...
\label{eq:damped}
\end{equation}
For small displacements from equilibrium with stiffness $K_i = \nabla^2\mathcal{F}|_{\mu^*}$, the discriminant $\Delta = \gamma_i^2 - 4K_i M_i$ determines three regimes: overdamped ($\Delta > 0$) with monotonic decay corresponding to standard Bayesian updating; critically damped ($\Delta = 0$) with fastest equilibration; and underdamped ($\Delta < 0$) with oscillatory approach, overshooting, and characteristic frequency $\omega = \sqrt{K_i/M_i - \gamma_i^2/4M_i^2}$.

\subsection{The Overdamped Limit}

In the overdamped limit $\gamma_i \to \infty$ (or equivalently $M_i \to 0$), the inertial term vanishes and we recover standard gradient descent $\gamma_i \dot{\mu}_i = -\nabla_{\mu_i}\mathcal{F}$ as in Friston's free energy principle. The free energy principle is thus the overdamped, high-friction, zero-inertia limit of a richer Hamiltonian theory. Neural systems may operate in this regime; however, ``it from bit'' physics requires the full second-order structure.
...
In the full gauge-theoretic framework \cite{Dennis2025a}, agents carry gauge frames $\phi_i(c) \in \mathfrak{so}(3)$ over the base manifold $\mathbb{C}$. The transport operator:
\begin{equation}
\Omega_{ij} = e^{\phi_i} e^{-\phi_j}
\end{equation}

...
naturally plays the role of an inertial mass tensor. Confident beliefs resist 
...
This article provides three primary contributions. First, we derive second-order belief dynamics from a variational free energy model, showing that the Fisher information metric provides a natural inertial mass tensor $M = \Lambda_{\text{prior}} + \Lambda_{\text{observation}} + \Lambda_{\text{social}}^{\text{in}} + \Lambda_{\text{social}}^{\text{out}}$ combining prior conviction, sensory data, and social interactions (both incoming influence and outgoing recoil). Via pullback geometry on statistical manifolds \citep{Amari2016,nielsen2020elementary}, we extend the variational free energy to multi-agent systems where social coupling takes the form $D_{\mathrm{KL}}(q_i \| \Omega_{ij} \cdot q_j)$, penalizing disagreement with neighbors after gauge transport into a common reference frame.
...
\textbf{Empirical validation.} Direct observation of underdamped belief oscillation, precision-scaled relaxation, or resonant persuasion remains for future experimental work. The Kaplowitz-Fink results \citep{kaplowitz1983, fink2002} provide suggestive evidence, but replication with controlled experiments utilizing the present framework's predictions would strengthen the empirical case.
